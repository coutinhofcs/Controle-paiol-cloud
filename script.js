...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('tabPanel').addEventListener('click', ()=>{ 
    ...
    window.currentEdit = null; // Limpa o currentEdit ao voltar para o painel
  });
  $('tabHist').addEventListener('click', ()=>{ 
    ...
    if (window.currentEdit) {
      loadFilteredHistory(window.currentEdit);
    } else {
      loadHistory();
    }
  });
  ...
  async function loadFilteredHistory(paiolId){
    histTableBody.innerHTML='';
    try{
  ...
  async function showHistoryForPaiol(paiolId){
    ...
    await loadFilteredHistory(paiolId);
    // Set tabs manually without triggering click event
    $('tabHist').classList.add('active');
    $('tabPanel').classList.remove('active');
    histArea.style.display='block';
    editor.style.display='none';
