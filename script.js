...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Modificação aqui: Ao clicar na aba de histórico, verifica se há um paiol selecionado
  $('tabHist').addEventListener('click', ()=>{ 
    ...
    if (window.currentEdit) {
      showHistoryForPaiol(window.currentEdit);
    } else {
  ...
  async function removePaiolAction(){
    ...
    // Substituído alert/confirm por um modal simples ou mensagem de ação para compatibilidade com iframe
  ...
  async function showHistoryForPaiol(paiolId){
    ...
    try{
      ...
      // Ativa a aba de histórico para mostrar os resultados
  ...
  async function exportHistoryCSV(all=true){
    try{
      ...
      if(all){
        ...
        // Substituído alert por showAction para consistência
      ...
      snap.forEach(docSnap => {
        ...
        // Substituir aspas duplas por aspas duplas escapadas para CSV
