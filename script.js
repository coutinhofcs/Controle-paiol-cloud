...
const firebaseConfig = {"apiKey": "AIzaSyAYvkpAe9Nfnn53JlSg8vOrKUFmUHn6oek", "authDomain": "controle-de-paiol.firebaseapp.com", "projectId": "controle-de-paiol", "storageBucket": "controle-de-paiol.firebasestorage.app", "messagingSenderId": "202438664461", "appId": "1:202438664461:web:c60d98f85fe8ab2ece9044", "measurementId": "G-BD3BR04LJQ"};
...
window.db = db;
window.auth = auth;
window.currentEdit = window.currentEdit || null;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('btnLogin').addEventListener('click', async () => {
    ...
    catch(e) { $('authMsg').textContent = e.message; console.error(e); }
  });

  $('btnRegister').addEventListener('click', async () => {
    try { const userCred = await createUserWithEmailAndPassword(auth, $('email').value.trim(), $('senha').value); await promptForNickname(userCred.user); }
    catch(e) { $('authMsg').textContent = e.message; console.error(e); }
  });
  ...
  async function promptForNickname(user) {
    ...
    } catch(e) { console.error('Falha ao salvar apelido:', e); }
  }

  function showAction(msg, type='ok') {
    actionMsg.innerHTML = '<div class="msg '+(type==='ok'?'ok':'err')+'">'+msg+'</div>';
    setTimeout(()=> actionMsg.innerHTML = '', 4000);
  }

  // Backend ops
  async function addPaiol(name) {
    if (!name) return showAction('Digite um nome', 'err');
    try {
  ...
  async function removePaiolAction() {
    ...
    if (!id) return showAction('Selecione um paiol', 'err');
    if (!confirm('Confirmar remoção do paiol?')) return;
    try {
      await deleteDoc(doc(db,'paiois', id));
      showAction('Paiol removido', 'ok');
    } catch(e) { showAction('Falha ao remover: '+e.message, 'err'); console.error(e); }
  }

  // Listeners: order by nome asc for alphabetical ordering
    ...
    try {
      ...
      window.unsubscribe = onSnapshot(q, snap => {
        ...
        snap.forEach(d => {
          ...
          div.innerHTML = `
            <div class='row'><strong>${escapeHtml(data.nome)}</strong><div class='right muted'>Última: ${lastInfo}</div></div>
  ...
  async function saveEditor() {
    ...
    try {
      ...
      await updateDoc(docRef, { tempAtual: temp, tempMax: max, tempMin: min, umid: umid, lacre: lacre, lastUpdate: serverTimestamp(), lastBy: (auth.currentUser && (auth.currentUser.displayName || auth.currentUser.email)) });
      await addDoc(collection(db,'history'), { t: serverTimestamp(), paiolId: currentEdit, paiolName: document.getElementById('editName').value, tempAtual: temp, tempMax: max, tempMin: min, umid: umid, lacre: lacre, user: (auth.currentUser && (auth.currentUser.displayName || auth.currentUser.email)) });
