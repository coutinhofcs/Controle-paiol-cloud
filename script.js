...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('tabPanel').addEventListener('click', ()=>{
    ...
    window.currentEdit = null; // Garante que currentEdit seja nulo ao voltar para o painel principal
  });

  // Ao clicar na aba de histórico, verifica se há um paiol selecionado
  $('tabHist').addEventListener('click', ()=>{
    ...
    // Se window.currentEdit contiver um ID de paiol, carrega o histórico específico
    if (window.currentEdit) {
      showHistoryForPaiol(window.currentEdit, false); // O segundo parâmetro 'false' indica para NÃO clicar na aba novamente
    } else {
      // Caso contrário, carrega o histórico completo
      loadHistory();
    }
  });
  ...
  async function promptForNickname(user){
    ...
    while(!nick){
      ...
      if(nick === null) break; // Usuário cancelou
      nick = (nick||'').trim();
    }
    if(nick){
  ...
  async function removePaiolAction(){
    ...
    if(!confirm("Confirmar remoção do paiol?")) return; // Idealmente, substituir por um modal personalizado
  ...
  async function startListeners(){
    ...
    try{
      ...
      window.unsubscribe = onSnapshot(q, snap => {
        ...
        // Modificação aqui: Ao clicar no botão de histórico, define o paiol e ativa a aba
        document.querySelectorAll('button[data-history]').forEach(b=> b.onclick = ()=>{
            window.currentEdit = b.getAttribute('data-history'); // Define o paiol atual
            $('tabHist').click(); // Ativa a aba de histórico, que por sua vez chamará showHistoryForPaiol
        });
  ...
  async function loadHistory(){
    histTableBody.innerHTML='';
    try{
      // Esta função carrega o histórico completo
  ...
  // O segundo parâmetro 'shouldClickTab' foi adicionado e removido o clique interno
  async function showHistoryForPaiol(paiolId, shouldClickTab = true){
    histTableBody.innerHTML='';
    currentEdit = paiolId; // Define currentEdit para que a exportação filtrada funcione
    window.currentEdit = currentEdit;
    try{
      // Esta função carrega o histórico para um paiol específico
      ...
      // Apenas ativa a aba de histórico se for um clique inicial de um botão
      if(shouldClickTab) {
  ...
  async function exportHistoryCSV(all=true){
    try{
      ...
      if(all){
        ...
        if(!pid){ showAction('Nenhum paiol selecionado. Abra o histórico do paiol antes de exportar filtrado.', 'warn'); return; }
        q = query(collection(db,'history'), where('paiolId','==', pid), orderBy('t','desc'));
      }
      const snap = await getDocs(q);
      if(snap.empty){ showAction('Histórico vazio. Nada para exportar.', 'warn'); return; }
      const rows = [];
    ...
    }catch(e){ console.error('Erro exportando histórico:', e); showAction('Falha ao exportar histórico: ' + (e && e.message ? e.message : e), 'err'); }
  }
